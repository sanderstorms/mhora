...
#region Windows Form Designer generated code
        ...
        private bool checkJhd(string fileName)
        {
            ...
            var h    = new Horoscope(info, new HoroscopeOptions());
            if (h.getPosition(Body.Body.Name.Ketu).toDivisionPosition(new Division(Basics.DivisionType.Rasi)).zodiac_house.value ==
                h.getPosition(Body.Body.Name.Lagna).toDivisionPosition(new Division(Basics.DivisionType.Rasi)).zodiac_house.value)
            {
                return true;
            }
