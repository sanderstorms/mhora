...
#region Component Designer generated code
    ...
    private void DrawItems(Graphics g, bool large)
    {
	    ...
	    foreach (var sign in signs)
	    {
		    ...
		    for (int item = 0; item < dpList.Count; item++)
		    {
                ...
                if (PrintMode == false)
                {
	                if ((dp.type == Elements.Body.Type.Graha) || (dp.type == Elements.Body.Type.Lagna))
	                {
    ...
    private void DrawItem(Graphics g, DivisionPosition dp, int item, bool large)
    {
        ...
        if (large)
        {
            ...
            if ((dp.type == Elements.Body.Type.Graha) || (dp.type == Elements.Body.Type.Lagna))
            {
				...
				if (dp.name == Elements.Body.Name.Lagna)
	            {
		            f = new Font(fBase.Name, fBase.Size, FontStyle.Bold);
	            }
                else if (bp.speed_longitude < 0.0 && bp.name != Elements.Body.Name.Rahu && bp.name != Elements.Body.Name.Ketu)
	            {
		...
		else
        {
            ...
            if (dp.type == Elements.Body.Type.BhavaArudhaSecondary)
            {
                fs = FontStyle.Italic;
            }
        ...
        if (dp.type == Elements.Body.Type.GrahaArudha)
        {
        ...
        switch (dp.type)
        {
            ...
            case Elements.Body.Type.GrahaArudha:
            ...
            case Elements.Body.Type.SpecialLagna:
                ...
                break;
            case Elements.Body.Type.BhavaArudha:
            case Elements.Body.Type.Varnada:
            case Elements.Body.Type.BhavaArudhaSecondary:
    ...
    private void PaintCharaKarakas(Graphics g)
    {
		...
		var dp2 = (DivisionPosition) div_pos[(int) Elements.Body.Name.Lagna];
        items.Add(dp2);
        DrawItems(g, true);
    }
    ...
    {
        var dpo = h.getPosition(Elements.Body.Name.Lagna).toDivisionPosition(options.Varga);
        items.Add(dpo);
    ...
    {
		var bItems = new Elements.Body.Name[10]
        {
        ...
        {
            if (options.ViewStyle == UserOptions.EViewStyle.Panchanga && dp.type != Elements.Body.Type.Graha)
            {
                continue;
            }
        ...
        foreach (DivisionPosition dp in div_pos)
        {
            if (dp.type != Elements.Body.Type.SpecialLagna)
            {
                continue;
            }
