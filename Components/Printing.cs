...
public class MhoraPrintDocument : PrintDocument
{
    ...
    protected override void OnBeginPrint(PrintEventArgs e)
    {
        ...
        for (var i = (int) Basics.DivisionType.HoraParasara; i <= (int) Basics.DivisionType.DwadasamsaDwadasamsa; i++)
        {
    ...
    private string GetDasaString(ToDate td, DasaEntry deAntar, bool bGraha)
    {
        ...
        if (bGraha)
        {
            s = string.Format("{0} {1}", Body.Body.toShortString(deAntar.graha), td.AddYears(deAntar.startUT).ToDateString());
        }
        else
        {
