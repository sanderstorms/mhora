...
using Mhora.Tables;
using Tithi = Mhora.Elements.Tithi;
...
#region Designer generated code
    ...
    private void ComputeEntry(double ut, double[] geopos)
    {
        ...
        if (opts.CalcLagnaCusps)
        {
            li = new ListViewItem();
            sweph.obtainLock(h);
            var bp_lagna_sr = Basics.CalculateSingleBodyPosition(ut_sr, sweph.BodyNameToSweph(Elements.Body.Name.Lagna), Elements.Body.Name.Lagna, Elements.Body.Type.Lagna, h);
            ...
            for (var i = 1; i <= 12; i++)
            {
                var r = new Retrogression(h, Elements.Body.Name.Lagna);
        ...
        {
            var bDiscard = true;
            var t        = new Mhora.Transit(h, Elements.Body.Name.Moon);
            sweph.obtainLock(h);
