    ...
    public class sweph
    {
        ...
        public static void obtainLock(Horoscope h)
        {
            ...
            lock (SwephLockObject)
            {
                ...
                swe_set_sid_mode((int)h.options.Ayanamsa, 0.0, 0.0);
            }
        }
        ...
        private static extern void xyz_swe_set_sid_mode(int sid_mode, double t0, double ayan_t0);
        ...
        private static extern double xyz_swe_revjul(double tjd, int gregflag, ref int year, ref int month, ref int day, ref double hour);
        ...
        private static extern int xyz_swe_calc_ut(double tjd_ut, int ipl, int iflag, double[] xx, StringBuilder serr);

        public static void swe_calc_ut(double tjd_ut, int ipl, int addFlags, double[] xx)
        {
            checkLock();
            var serr = new StringBuilder(256);
            var ret  = xyz_swe_calc_ut(tjd_ut, ipl, iflag | addFlags, xx, serr);
            if (ret < 0)
            {
        ...
        private static extern int xyz_swe_sol_eclipse_when_glob(double tjd_ut, int iflag, int ifltype, double[] tret, bool backward, StringBuilder s);

        public static void swe_sol_eclipse_when_glob(double tjd_ut, double[] tret, bool forward)
        {
            checkLock();
            var serr = new StringBuilder(256);
            var ret  = xyz_swe_sol_eclipse_when_glob(tjd_ut, iflag, 0, tret, !forward, serr);
            if (ret < 0)
            {
                Console.WriteLine("Sweph Error: {0}", serr);
                throw new SwephException(serr.ToString());
        ...
        private static extern int xyz_swe_sol_eclipse_when_loc(double tjd_ut, int iflag, double[] geopos, double[] tret, double[] attr, bool backward, StringBuilder s);
        ...
        public static void swe_sol_eclipse_when_loc(HoraInfo hi, double tjd_ut, double[] tret, double[] attr, bool forward)
        {
            ...
            var ret = xyz_swe_sol_eclipse_when_loc(tjd_ut, iflag, geopos, tret, attr, !forward, serr);
            if (ret < 0)
            {
                Console.WriteLine("Sweph Error: {0}", serr);
                throw new SwephException(serr.ToString());
        ...
        private static extern int xyz_swe_lun_eclipse_when(double tjd_ut, int iflag, int ifltype, double[] tret, bool backward, StringBuilder s);

        public static void swe_lun_eclipse_when(double tjd_ut, double[] tret, bool forward)
        {
            checkLock();
            var serr = new StringBuilder(256);
            var ret  = xyz_swe_lun_eclipse_when(tjd_ut, iflag, 0, tret, !forward, serr);
            if (ret < 0)
            {
                Console.WriteLine("Sweph Error: {0}", serr);
                throw new SwephException(serr.ToString());
        ...
        private static extern int xyz_swe_lun_occult_when_loc(
            double        tjd_ut,
            int           ipl,
            ref string    starname,
            int           iflag,
        ...
        private static extern double xyz_swe_get_ayanamsa_ut(double tjd_ut);

        public static double swe_get_ayanamsa_ut(double tjd_ut)
        {
            checkLock();
            return xyz_swe_get_ayanamsa_ut(tjd_ut);
        }

        [DllImport("swedll64", CharSet = CharSet.Ansi, EntryPoint = "swe_rise_trans")]
        private static extern int xyz_swe_rise_trans(double tjd_ut, int ipl, string starname, int epheflag, int rsmi, double[] geopos, double atpress, double attemp, double[] tret, StringBuilder serr);

        public static void swe_rise(double tjd_ut, int ipl, int rsflag, double[] geopos, double atpress, double attemp, double[] tret)
        {
            checkLock();
            var serr = new StringBuilder(256);
            var ret  = xyz_swe_rise_trans(tjd_ut, ipl, string.Empty, iflag, SE_CALC_RISE | rsflag, geopos, atpress, attemp, tret, serr);
            if (ret < 0)
            {
        ...
        public static void swe_set(double tjd_ut, int ipl, int rsflag, double[] geopos, double atpress, double attemp, double[] tret)
        {
            checkLock();
            var serr = new StringBuilder(256);
            var ret = xyz_swe_rise_trans(tjd_ut,
                                         ipl,
                                         string.Empty,
                                         iflag,
                                         SE_CALC_SET | rsflag,
        ...
        private static extern int xyz_swe_houses_ex(double tjd_ut, int iflag, double lat, double lon, int hsys, double[] cusps, double[] ascmc);

        public static int swe_houses_ex(double tjd_ut, int iflag, double lat, double lon, int hsys, double[] cusps, double[] ascmc)
        {
            checkLock();
            var ret = xyz_swe_houses_ex(tjd_ut, iflag, lat, lon, hsys, cusps, ascmc);
        ...
        public static double swe_lagna(double tjd_ut)
        {
            ...
            var ret   = swe_houses_ex(tjd_ut, SEFLG_SIDEREAL, hi.lat.toDouble(), hi.lon.toDouble(), 'R', cusps, ascmc);
            return ascmc[0];
        }
        ...
        private static extern void swe_set_tid_acc(double t_acc);
