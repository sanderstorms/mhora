...
public class AshtakavargaControl : MhoraControl
{
	...
	private readonly Tables.Body.Name[]  innerBodies;
	...
	private Tables.Body.Name[] outerBodies;
	...
	public AshtakavargaControl(Horoscope _h)
	{
		...
		innerBodies = (Tables.Body.Name[]) outerBodies.Clone();
...
#region Designer generated code
	...
	private void DrawJhoraChakra(Graphics g)
	{
		...
		var        lagna  = h.getPosition(Tables.Body.Name.Lagna).longitude.toZodiacHouse();
		var        offset = 5;
		...
		bins[1] = av.getPav(Tables.Body.Name.Lagna);
		bins[2] = av.getPav(Tables.Body.Name.Sun);
		bins[3] = av.getPav(Tables.Body.Name.Moon);
		bins[4] = av.getPav(Tables.Body.Name.Mars);
		bins[5] = av.getPav(Tables.Body.Name.Mercury);
		bins[6] = av.getPav(Tables.Body.Name.Jupiter);
		bins[7] = av.getPav(Tables.Body.Name.Venus);
		bins[8] = av.getPav(Tables.Body.Name.Saturn);

		var strs = new string[9];
		strs[0] = "SAV";
		strs[1] = Tables.Body.toString(Tables.Body.Name.Lagna);
		strs[2] = Tables.Body.toString(Tables.Body.Name.Sun);
		strs[3] = Tables.Body.toString(Tables.Body.Name.Moon);
		strs[4] = Tables.Body.toString(Tables.Body.Name.Mars);
		strs[5] = Tables.Body.toString(Tables.Body.Name.Mercury);
		strs[6] = Tables.Body.toString(Tables.Body.Name.Jupiter);
		strs[7] = Tables.Body.toString(Tables.Body.Name.Venus);
		strs[8] = Tables.Body.toString(Tables.Body.Name.Saturn);
	...
	private void DrawChanchaInner(Graphics g)
	{
		...
		if (outerBodies.Length == 1)
		{
			var desc = Tables.Body.toString(outerBodies[0]);
