...
namespace Mhora.Varga
{
    ...
    public class VargaRectificationForm : Form
    {
        ...
        private readonly Division          dtypeRasi = new Division(Basics.DivisionType.Rasi);
        ...
        private void PopulateCache()
        {
            ...
            for (var i = 0; i < opts.Divisions.Length; i++)
            {
                ...
                sweph.obtainLock(h);
                var bp = Basics.CalculateSingleBodyPosition(ut_curr,
                                                            sweph.BodyNameToSweph(mBody),
                                                            mBody,
                                                            BodyType.Name.Graha,
                                                            h);
                sweph.releaseLock(h);
                ...
                for (var j = 0; j < momentCusps[iVarga].Length; j++)
                {
                    ...
                    if (old_x_offset + sz.Width < x_offset)
                    {
                        ...
                        g.DrawString(s,
                                     ...
                                     Brushes.Gray,
                                     x_offset - sz.Width / 2,
                                     varga_y  - sz.Height - half_tick_height);
