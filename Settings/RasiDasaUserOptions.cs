...
public class RasiDasaUserOptions : ICloneable
{
	...
	protected Tables.Body.Name      mCoLordAqu;
	protected Tables.Body.Name      mCoLordSco;
	...
	public Tables.Body.Name ColordAqu
	{
		get => mCoLordAqu;
	...
	public Tables.Body.Name ColordSco
	{
		get => mCoLordSco;
	...
	{
		mSeed      = h.getPosition(Tables.Body.Name.Lagna).toDivisionPosition(Division).zodiac_house.value;
		mSeedHouse = 1;
	}
	...
	public void calculateCoLords()
	{
		...
		mCoLordAqu = fs.StrongerGraha(Tables.Body.Name.Saturn, Tables.Body.Name.Rahu, true);
		mCoLordSco = fs.StrongerGraha(Tables.Body.Name.Mars, Tables.Body.Name.Ketu, true);
	}
	...
	public void calculateExceptions()
	{
		...
		var zhKetu = h.getPosition(Tables.Body.Name.Ketu).toDivisionPosition(Division).zodiac_house.value;
		var zhSat  = h.getPosition(Tables.Body.Name.Saturn).toDivisionPosition(Division).zodiac_house.value;
		...
		else
		{
			...
			var b  = fs.StrongerGraha(Tables.Body.Name.Saturn, Tables.Body.Name.Ketu, false);
			if (b == Tables.Body.Name.Ketu)
			{
				mKetuExceptions.houses.Add(zhKetu);
			}
			else
