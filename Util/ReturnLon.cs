...
{
    private readonly Elements.Body.Name b;
    private readonly Horoscope          h;

    public NonLinearTransit(Horoscope _h, Elements.Body.Name _b)
    {
        h = _h;
        b = _b;
    }

    public int BodyNameToSweph(Elements.Body.Name b)
    {
        switch (b)
        {
            case Elements.Body.Name.Sun:     return sweph.SE_SUN;
            case Elements.Body.Name.Moon:    return sweph.SE_MOON;
            case Elements.Body.Name.Mars:    return sweph.SE_MARS;
            case Elements.Body.Name.Mercury: return sweph.SE_MERCURY;
            case Elements.Body.Name.Jupiter: return sweph.SE_JUPITER;
            case Elements.Body.Name.Venus:   return sweph.SE_VENUS;
            case Elements.Body.Name.Saturn:  return sweph.SE_SATURN;
    ...
    public Longitude GetLongitude(double ut, ref bool bForwardDir)
    {
        ...
        var bp        = Basics.CalculateSingleBodyPosition(ut, swephBody, b, Elements.Body.Type.Other, h);
...
public class Retrogression
{
    ...
    public Retrogression(Horoscope _h, Elements.Body.Name _b)
    {
    ...
    {
        var bp = Basics.CalculateSingleBodyPosition(ut, sweph.BodyNameToSweph(b), b, Elements.Body.Type.Other, h);
        while (Transit.CircLonLessThan(bp.longitude, lonToFind))
        {
            //mhora.Log.Debug("- {0} {1}", bp.longitude.value, lonToFind.value);
            ut++;
            bp = Basics.CalculateSingleBodyPosition(ut, sweph.BodyNameToSweph(b), b, Elements.Body.Type.Other, h);
        }
    ...
    public double GetTransitBackward(double ut, Longitude lonToFind)
    {
        ...
        while (true)
        {
            ...
            var bp_next = Basics.CalculateSingleBodyPosition(ut_curr, sweph.BodyNameToSweph(b), b, Elements.Body.Type.Other, h);
            var bp_curr = Basics.CalculateSingleBodyPosition(ut_next, sweph.BodyNameToSweph(b), b, Elements.Body.Type.Other, h);
    ...
    public double GetTransitForward(double ut, Longitude lonToFind)
    {
        ...
        while (true)
        {
            ...
            var bp_curr = Basics.CalculateSingleBodyPosition(ut_curr, sweph.BodyNameToSweph(b), b, Elements.Body.Type.Other, h);
            var bp_next = Basics.CalculateSingleBodyPosition(ut_next, sweph.BodyNameToSweph(b), b, Elements.Body.Type.Other, h);
    ...
    {
        var ut_step = 5.0;
        var bp      = Basics.CalculateSingleBodyPosition(start_ut, sweph.BodyNameToSweph(b), b, Elements.Body.Type.Other, h);
        ...
        if (bp.speed_longitude >= 0)
        {
            ...
            while (true)
            {
                ...
                // Find speeds
                var bp_l = Basics.CalculateSingleBodyPosition(lower_ut, sweph.BodyNameToSweph(b), b, Elements.Body.Type.Other, h);
                var bp_h = Basics.CalculateSingleBodyPosition(higher_ut, sweph.BodyNameToSweph(b), b, Elements.Body.Type.Other, h);
...
public class Transit
{
    ...
    {
        h = _h;
        b = Elements.Body.Name.Other;
    }

    public Transit(Horoscope _h, Elements.Body.Name _b)
    {
        h = _h;
        b = _b;
    }
    ...
    {
        var bp = Basics.CalculateSingleBodyPosition(ut, sweph.SE_SUN, Elements.Body.Name.Sun, Elements.Body.Type.Graha, h);
        if (bp.speed_longitude >= 0)
        {
            bDirRetro = false;
        }
    ...
    {
        var bp_sun  = Basics.CalculateSingleBodyPosition(ut, sweph.SE_SUN, Elements.Body.Name.Sun, Elements.Body.Type.Graha, h);
        var bp_moon = Basics.CalculateSingleBodyPosition(ut, sweph.SE_MOON, Elements.Body.Name.Moon, Elements.Body.Type.Graha, h);
    ...
    public double NonLinearSearch(double ut, Elements.Body.Name b, Longitude lon_to_find, ReturnLon func)
    {
...
internal class CuspTransitSearch
{
    ...
    public double TransitSearchDirect(Elements.Body.Name SearchBody, Moment StartDate, bool Forward, Longitude TransitPoint, Longitude FoundLon, ref bool bForward)
    {
        var bDiscard = true;
        ...
        if (SearchBody == Elements.Body.Name.Lagna)
        {
    ...
    public double TransitSearch(Elements.Body.Name SearchBody, Moment StartDate, bool Forward, Longitude TransitPoint, Longitude FoundLon, ref bool bForward)
    {
        if (SearchBody == Elements.Body.Name.Sun || SearchBody == Elements.Body.Name.Moon)
        {
            return TransitSearchDirect(SearchBody, StartDate, Forward, TransitPoint, FoundLon, ref bForward);
        }

        if (((int) SearchBody <= (int) Elements.Body.Name.Moon || (int) SearchBody > (int) Elements.Body.Name.Saturn) && SearchBody != Elements.Body.Name.Lagna)
        {
