...
namespace Mhora.Util
{
    public class ToDate
    {
        ...
        public ToDate(double _baseUT, DateType _type, double _yearLength, double _compression, Horoscope _h)
        {
            ...
            compression = _compression;
            h           = _h;
            spos        = h.getPosition(Body.Body.Name.Sun).longitude.value;
            mpos        = h.getPosition(Body.Body.Name.Moon).longitude.value;
        }
                                      ...
                                      int       numDays)
        {
            ...
            t       = new Transit(h);
            soff    = h.getPosition(Body.Body.Name.Sun).longitude.toZodiacHouseOffset();
            _years  = years;
            tYears  = 0;
            tMonths = 0;
            tDays   = 0;
