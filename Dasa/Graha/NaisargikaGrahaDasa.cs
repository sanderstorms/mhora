    ...
    public class NaisargikaGrahaDasa : Dasa, IDasa
    {
        ...
        public ArrayList Dasa(int cycle)
        {
            ...
            foreach (var bn in order)
            {
                ...
                al.Add(new DasaEntry(bn, cycle_start + curr, dasaLength, 1, Body.Body.toShortString(bn)));
                curr += dasaLength;
            }
        ...
        public ArrayList AntarDasa(DasaEntry pdi)
        {
            ...
            {
                foreach (Body.Body.Name b in options.GrahaStrengths[i].grahas)
                {
            ...
            {
                if (ExcludeGraha(pdi, (Body.Body.Name)orderedAntar.grahas[i]))
                {
                    continue;
                }

                var diff = lzh.numHousesBetween(h.getPosition((Body.Body.Name)orderedAntar.grahas[i]).toDivisionPosition(options.dtype).zodiac_house);
                switch (diff)
                {
                    case 7:
                        antarLengths[i]   =  12.0;
            ...
            for (var i = 0; i < size; i++)
            {
                var bn = (Body.Body.Name)orderedAntar.grahas[i];
                ...
                var desc   = pdi.shortDesc + " " + Body.Body.toShortString(bn);
        ...
        public double lengthOfDasa(Body.Body.Name plt)
        {
            switch (plt)
            {
                case Body.Body.Name.Sun:     return 20;
                case Body.Body.Name.Moon:    return 1;
                case Body.Body.Name.Mars:    return 2;
                case Body.Body.Name.Mercury: return 9;
                case Body.Body.Name.Jupiter: return 18;
                case Body.Body.Name.Venus:   return 20;
                case Body.Body.Name.Saturn:  return 50;
                case Body.Body.Name.Lagna:   return 0;
            }
        ...
        private bool ExcludeGraha(DasaEntry pdi, Body.Body.Name graha)
        {
            if (options.ExcludeDasaLord &&
                graha == pdi.graha)
            {
                return true;
            }
            ...
            if (options.ExcludeNodes &&
                (graha == Body.Body.Name.Rahu ||
                 graha == Body.Body.Name.Ketu))
            {
                return true;
            }
        ...
        public class UserOptions : ICloneable
        {
            ...
            private readonly Horoscope h;
            protected Body.Body.Name mLordAqu;
            protected Body.Body.Name mLordSco;
            ...
            public UserOptions(Horoscope _h)
            {
                ...
                mLordSco          = fs.StrongerGraha(Body.Body.Name.Mars, Body.Body.Name.Ketu, true);
                mLordAqu          = fs.StrongerGraha(Body.Body.Name.Saturn, Body.Body.Name.Rahu, true);
                ExcludeNodes      = true;
            ...
            public Body.Body.Name Lord_Sco
            {
                get =>
                    mLordSco;
                set =>
            ...
            public Body.Body.Name Lord_Aqu
            {
                get =>
                    mLordAqu;
                set =>
            ...
            {
                if (newOpts.mLordAqu != mLordAqu ||
                    newOpts.mLordSco != mLordSco)
                {
                ...
                for (var i = 0; i < 3; i++)
                {
                    ...
                    {
                        if ((ZodiacHouse.Name)newOpts.RasiStrengths[i].houses[j] !=
                            (ZodiacHouse.Name)RasiStrengths[i].houses[j])
                        {
                            newOpts.CalculateGrahaStrengths();
                            return;
                        }
            ...
            public void CalculateRasiStrengths()
            {
                ...
                RasiStrengths = fs.ResultsZodiacKendras(h.CalculateDivisionPosition(h.getPosition(Body.Body.Name.Lagna), dtype).zodiac_house.value);
            }
            ...
            public void CalculateGrahaStrengths()
            {
                ...
                for (var i = 0; i < RasiStrengths.Length; i++)
                {
                    ...
                    foreach (ZodiacHouse.Name zn in oz.houses)
                    {
                        ...
                        var temp_arr = new Body.Body.Name[temp.Count];
                        for (var j = 0; j < temp.Count; j++)
                        {
                            temp_arr[j] = (Body.Body.Name)temp[j];
                        }
