...
public class NaisargikaGrahaDasa : Dasa, IDasa
{
	...
	public ArrayList Dasa(int cycle)
	{
		...
		foreach (var bn in order)
		{
			...
			al.Add(new DasaEntry(bn, cycle_start + curr, dasaLength, 1, Tables.Body.toShortString(bn)));
			curr += dasaLength;
		}
	...
	public ArrayList AntarDasa(DasaEntry pdi)
	{
		...
		{
			foreach (Tables.Body.Name b in options.GrahaStrengths[i].grahas)
			{
		...
		{
			if (ExcludeGraha(pdi, (Tables.Body.Name) orderedAntar.grahas[i]))
			{
				continue;
			}

			var diff = lzh.numHousesBetween(h.getPosition((Tables.Body.Name) orderedAntar.grahas[i]).toDivisionPosition(options.dtype).zodiac_house);
			switch (diff)
			{
				case 7:
					antarLengths[i]   =  12.0;
		...
		for (var i = 0; i < size; i++)
		{
			var bn = (Tables.Body.Name) orderedAntar.grahas[i];
			...
			var desc   = pdi.shortDesc + " " + Tables.Body.toShortString(bn);
	...
	public double lengthOfDasa(Tables.Body.Name plt)
	{
		switch (plt)
		{
			case Tables.Body.Name.Sun:     return 20;
			case Tables.Body.Name.Moon:    return 1;
			case Tables.Body.Name.Mars:    return 2;
			case Tables.Body.Name.Mercury: return 9;
			case Tables.Body.Name.Jupiter: return 18;
			case Tables.Body.Name.Venus:   return 20;
			case Tables.Body.Name.Saturn:  return 50;
			case Tables.Body.Name.Lagna:   return 0;
		}
	...
	private bool ExcludeGraha(DasaEntry pdi, Tables.Body.Name graha)
	{
		...
		if (options.ExcludeNodes && (graha == Tables.Body.Name.Rahu || graha == Tables.Body.Name.Ketu))
		{
			return true;
		}
	...
	public class UserOptions : ICloneable
	{
		...
		protected        Tables.Body.Name mLordAqu;
		protected        Tables.Body.Name mLordSco;
		...
		public UserOptions(Horoscope _h)
		{
			...
			mLordSco          = fs.StrongerGraha(Tables.Body.Name.Mars, Tables.Body.Name.Ketu, true);
			mLordAqu          = fs.StrongerGraha(Tables.Body.Name.Saturn, Tables.Body.Name.Rahu, true);
			ExcludeNodes      = true;
		...
		public Tables.Body.Name Lord_Sco
		{
			get => mLordSco;
			set => mLordSco = value;
		}
		...
		public Tables.Body.Name Lord_Aqu
		{
			get => mLordAqu;
			set => mLordAqu = value;
		}
		...
		public void CalculateRasiStrengths()
		{
			...
			RasiStrengths = fs.ResultsZodiacKendras(h.CalculateDivisionPosition(h.getPosition(Tables.Body.Name.Lagna), dtype).zodiac_house.value);
		}
		...
		public void CalculateGrahaStrengths()
		{
			...
			for (var i = 0; i < RasiStrengths.Length; i++)
			{
				...
				foreach (ZodiacHouse.Name zn in oz.houses)
				{
					...
					var temp_arr = new Tables.Body.Name[temp.Count];
					for (var j = 0; j < temp.Count; j++)
					{
						temp_arr[j] = (Tables.Body.Name) temp[j];
					}
