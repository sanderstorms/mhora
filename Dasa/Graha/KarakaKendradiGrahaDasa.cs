...
namespace Mhora
{
    public class KarakaKendradiGrahaDasa : Dasa, IDasa
    {
        ...
        public ArrayList Dasa(int cycle)
        {
            ...
            foreach (Body.Body.Name b in options.GrahaStrengths.grahas)
            {
                var dasaLength = lengthOfDasa(b);
                al.Add(new DasaEntry(b, cycle_start + curr, dasaLength, 1, Body.Body.toShortString(b)));
                curr += dasaLength;
            }
            ...
            for (var i = 0; i < numDasas; i++)
            {
                ...
                al.Add(new DasaEntry(de.graha, cycle_start + curr, dasaLength, 1, Body.Body.toShortString(de.graha)));
                curr += dasaLength;
            }
        ...
        public ArrayList AntarDasa(DasaEntry pdi)
        {
            ...
            foreach (Body.Body.Name b in bOrder)
            {
                al.Add(new DasaEntry(b, curr, dasaLength, pdi.level + 1, pdi.shortDesc + " " + Body.Body.toShortString(b)));
                curr += dasaLength;
            }
        ...
        public static double LengthOfDasa(Horoscope h, Division dtype, Body.Body.Name plt, DivisionPosition dp_plt)
        {
            double length = 0;
            ...
            if (plt == Body.Body.Name.Rahu)
            {
                zh_mt.value = ZodiacHouse.Name.Aqu;
            }

            if (plt == Body.Body.Name.Ketu)
            {
        ...
        public class UserOptions : ICloneable
        {
            ...
            private readonly Horoscope h;
            protected Body.Body.Name mSeedBody;
            ...
            public Body.Body.Name SeedBody
            {
                get =>
                    mSeedBody;
                set =>
            ...
            public void CalculateSeedBody()
            {
                ...
                for (var i = (int)Body.Body.Name.Sun; i <= (int)Body.Body.Name.Rahu; i++)
                {
            ...
            public void CalculateRasiStrengths()
            {
                ...
                var zh_sat = h.getPosition(Body.Body.Name.Saturn).toDivisionPosition(new Division(Basics.DivisionType.Rasi)).zodiac_house.value;
                var zh_ket = h.getPosition(Body.Body.Name.Ketu).toDivisionPosition(new Division(Basics.DivisionType.Rasi)).zodiac_house.value;
                ...
                else if (zh_sat == zh_ket && zh_sat == zh.value)
                {
                    ...
                    bIsForward = fs2.CmpGraha(Body.Body.Name.Saturn, Body.Body.Name.Ketu, false);
                }
            ...
            public void CalculateGrahaStrengths()
            {
                ...
                foreach (var oz in RasiStrengths)
                {
                    foreach (ZodiacHouse.Name zn in oz.houses)
                    {
                        ...
                        {
                            temp_arr[i] = (Body.Body.Name)temp[i];
                        }
