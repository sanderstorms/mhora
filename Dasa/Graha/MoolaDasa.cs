...
public class MoolaDasa : Dasa, IDasa
{
	...
	public ArrayList Dasa(int cycle)
	{
		...
		foreach (Tables.Body.Name b in options.GrahaStrengths.grahas)
		{
			var dasaLength = lengthOfDasa(b);
			al.Add(new DasaEntry(b, cycle_start + curr, dasaLength, 1, Tables.Body.toShortString(b)));
			curr += dasaLength;
		}
		...
		for (var i = 0; i < numDasas; i++)
		{
			...
			al.Add(new DasaEntry(de.graha, cycle_start + curr, dasaLength, 1, Tables.Body.toShortString(de.graha)));
			curr += dasaLength;
		}
	...
	public ArrayList AntarDasa(DasaEntry pdi)
	{
		...
		foreach (Tables.Body.Name b in bOrder)
		{
			var dasaLength = vd.lengthOfDasa(b) / vd.paramAyus() * pdi.dasaLength;
			al.Add(new DasaEntry(b, curr, dasaLength, pdi.level + 1, pdi.shortDesc + " " + Tables.Body.toShortString(b)));
			curr += dasaLength;
		}
	...
	public double lengthOfDasa(Tables.Body.Name plt)
	{
		...
		if (plt == Tables.Body.Name.Rahu)
		{
			zh_mt.value = ZodiacHouse.Name.Aqu;
		}

		if (plt == Tables.Body.Name.Ketu)
		{
	...
	public class UserOptions : ICloneable
	{
		...
		protected        Tables.Body.Name mSeedBody;
		...
		public UserOptions(Horoscope _h)
		{
			...
			mSeedBody   = Tables.Body.Name.Lagna;
		...
		public Tables.Body.Name SeedBody
		{
			get => mSeedBody;
			set => mSeedBody = value;
		}
		...
		public void CalculateRasiStrengths()
		{
			...
			var zh_sat = h.getPosition(Tables.Body.Name.Saturn).toDivisionPosition(new Division(Basics.DivisionType.Rasi)).zodiac_house.value;
			var zh_ket = h.getPosition(Tables.Body.Name.Ketu).toDivisionPosition(new Division(Basics.DivisionType.Rasi)).zodiac_house.value;
			...
			else if (zh_sat == zh_ket && zh_sat == zh.value)
			{
				...
				bIsForward = fs2.CmpGraha(Tables.Body.Name.Saturn, Tables.Body.Name.Ketu, false);
			}
		...
		public void CalculateGrahaStrengths()
		{
			...
			foreach (var oz in RasiStrengths)
			{
				foreach (ZodiacHouse.Name zn in oz.houses)
				{
					...
					{
						temp_arr[i] = (Tables.Body.Name) temp[i];
					}
