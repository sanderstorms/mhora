...
public class NakshatraPraveshDasa : Dasa, IDasa
{
    ...
    public ArrayList Dasa(int cycle)
    {
        ...
        for (var i = 0; i < 60; i++)
        {
            ...
            var di    = new DasaEntry(Tables.Body.Name.Other, start, 1.0, 1, "Nakshatra Pravesh Year");
            al.Add(di);
        }
    ...
    public new string EntryDescription(DasaEntry pdi, Moment start, Moment end)
    {
        ...
        if (pdi.level == 3)
        {
            var l = Basics.CalculateBodyLongitude(start.toUniversalTime(), sweph.BodyNameToSweph(Tables.Body.Name.Moon));
            var n = l.toNakshatra();
