...
public class Position : ICloneable
{
    ...
    public Position(Horoscope _h, Elements.Body.Name aname, Elements.Body.Type atype, Longitude lon, double lat, double dist, double splon, double splat, double spdist)
    {
        longitude       = lon;
        latitude        = lat;
        distance        = dist;
    ...
    private DivisionPosition toBhavaDivisionPositionHouse(Longitude[] cusps)
    {
        ...
        var zlagna = h.getPosition(Elements.Body.Name.Lagna).toDivisionPosition(new Division(Basics.DivisionType.Rasi)).zodiac_house;
        for (var i = 0; i < 12; i++)
        {
            if (longitude.sub(cusps[i]).value < cusps[i + 1].sub(cusps[i]).value)
            {
    ...
    {
        var offset = h.getPosition(Elements.Body.Name.Lagna).longitude.toZodiacHouseOffset();
