...
#region Designer generated code
        ...
        {
            if (opts.SearchBody != Body.Body.Name.Sun &&
                opts.SearchBody != Body.Body.Name.Moon) // &&
            ...
            {
                curr_julday = t.LinearSearch(curr_julday + DirectSpeed(opts.SearchBody),
                                             start_lon,
                                             t.GenericLongitude);
                totalProgression -= 360.0;
            }

            curr_julday = t.LinearSearch(curr_julday + totalProgression / 360.0 * DirectSpeed(opts.SearchBody),
                                         start_lon.add(totalProgression),
                                         t.GenericLongitude);
        ...
        {
            if ((int)opts.SearchBody <= (int)Body.Body.Name.Moon ||
                (int)opts.SearchBody > (int)Body.Body.Name.Saturn)
            {
                DirectProgression();
                return;
            }
        ...
        private void bRetroCusp_Click(object sender, EventArgs e)
        {
            ...
            var          fmt = hTransit.info.DateOfBirth.ToString();
            ListViewItem li  = new TransitItem(hTransit);
            li.Text = Body.Body.toString(opts.SearchBody);
            if (becomesDirect)
            {
        ...
        private double StartSearch(bool bUpdateDate)
        {
            ...
            var bForward  = true;
            var found_ut =
                cs.TransitSearch(opts.SearchBody,
                                 opts.StartDate,
                                 opts.Forward,
                                 opts.TransitPoint,
                                 found_lon,
        ...
        {
            opts.SearchBody         = Body.Body.Name.Sun;
        ...
        private void bVargaChange_Click(object sender, EventArgs e)
        {
            if (opts.SearchBody == Body.Body.Name.Sun  ||
                opts.SearchBody == Body.Body.Name.Moon ||
                opts.SearchBody == Body.Body.Name.Lagna)
            {
                if (opts.Forward)
                {
