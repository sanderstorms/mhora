...
#region Designer generated code
	...
	{
		if (opts.SearchBody != Tables.Body.Name.Sun && opts.SearchBody != Tables.Body.Name.Moon) // &&
	...
	{
		if ((int) opts.SearchBody <= (int) Tables.Body.Name.Moon || (int) opts.SearchBody > (int) Tables.Body.Name.Saturn)
		{
			DirectProgression();
			return;
		}
	...
	private void bRetroCusp_Click(object sender, EventArgs e)
	{
		...
		var          fmt = hTransit.info.DateOfBirth.ToString();
		ListViewItem li  = new TransitItem(hTransit);
		li.Text = Tables.Body.toString(opts.SearchBody);
		if (becomesDirect)
		{
	...
	{
		opts.SearchBody         = Tables.Body.Name.Sun;
	...
	{
		if (opts.SearchBody == Tables.Body.Name.Sun || opts.SearchBody == Tables.Body.Name.Moon || opts.SearchBody == Tables.Body.Name.Lagna)
		{
			if (opts.Forward)
			{
